        ...
        <div className='flex sm:w-full lg:w-[70%] flex-col bg-white rounded-lg shadow-lg p-6'>
            <div className='flex flex-row justify-between items-center w-full mb-6 pb-4 border-b border-gray-200'>
                <h2 className='text-2xl font-bold text-gray-800'>
                    Koszyk ({products.length} {products.length === 1 ? 'produkt' : products.length >= 2 && products.length <= 4 ? 'produkty' : 'produktów'})
                </h2>
                <button
                    onClick={() => setCheckout([])}
            ...
            <div className='space-y-4'>
                {groupedProducts.length > 0 ? (
                    groupedProducts.map((product) => (
                        <div key={product.id} className='flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 hover:shadow-md transition-shadow duration-200'>
                            ...
                            <div className='flex items-center gap-4'>
                                ...
                                <div className='text-right'>
                                    <p className='text-lg font-bold text-blue-600'>{(getPrice(product.price) * product.quantity).toFixed(2)} zł</p>
                    ...
                    <div className='text-center py-12'>
                        ...
                        <p className='text-gray-400'>Dodaj produkty, aby kontynuować zakupy</p>
                    </div>
                )}
            </div>
        </div>
